...
| respawn 후 재적용 (~10:11부터) | **재적용 → 백색 재진입** | 세션 101100=185KB(부분)→101200~101600 519~534KB 연속 백색, 센서 16× 실측. 트리거 = **동거 세션의 10:09Z gstApp 하네스(cam-operate 재기동+하드 리셋, ESTALE×2 후 3차 COLD_PREPARED 복구)** — 콜드 경로 재시작 직후부터 백색 (당초 "10:16, 트리거 미상"은 tail 절단 관측 아티팩트). 순수 "자발" 재적용 증거 없음, 공백 0 |
...
**잔여 해소 (2026-08-21 보드 공유 알림):** 10:16 재적용의 트리거는 동거
세션의 gstApp 하네스 시나리오(10:09Z cam-operate 재기동+하드 리셋)로
판명 — 콜드 경로 재시작 = race 모델 부합. 순수 자발 재적용 증거 없음.
FW 수정 검증 시 "재적용이 라이프사이클 이벤트로만 발생하는지" 확인으로
축소 (#26 코멘트 정정 완료). 동거 세션의 10:47~11:22Z 비출하 유닛
(cam-prepare-warm) 오염 창은 본 문서의 어떤 측정과도 겹치지 않음을
교차 확인함 (백로그 재검증은 11:22 원복 후 실행).
