...
- 가드: `tests/test_pr_reviews.py` 44건(+ subtest 3건). 픽스처는 실제 PR 페이로드에서 잘라온
  ...
  신뢰 구성원의 Codex 인라인 답글도 처분으로 인정하되, 다른 review comment는
  새 지적일 수 있어 제외한다. Sticky 리뷰가 처분 후 갱신되면 새 본문 확인을 위해
  예전 처분을 무효화하고, STALE과 FINDINGS가 겹치면 표에 두 상태를 함께 표시한다.
