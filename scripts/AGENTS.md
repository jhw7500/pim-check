...
| `guard_pim_board_command.py` | Claude Bash PreToolUse 가드. `env` option cluster·`builtin`·`exec`·`eval`·`source`·`nice`·`stdbuf`·`xargs`·`find`·`watch`·`taskset`·`chrt`·`setsid`·`sudo` 등 launcher·shell `-c`·Bash ANSI-C quote·stdin shell fail-closed·명령 치환·그룹/조건 제어문 내부의 직접 plan/standalone runner 실행도 차단한다. `source`/`.`의 선택적 `--` 뒤 runner, `find`의 실행 action과 placeholder 가능성, `watch -x`의 exec 자식과 기본 shell command, `taskset`의 affinity 및 `chrt`의 priority 뒤 실행 자식을 재귀 검사하며 정확한 저장소 wrapper 경로만 면제한다. 앞선 `cd`·`pushd`·`popd` 뒤에는 상대 wrapper 경로를 면제하지 않고 절대 정본 경로만 허용한다. PID 모드는 실행 자식 없음으로 구분하고, ANSI-C escape·미종결 action·비정규 경로·축약 불가능한 복합 문법은 fail-closed 처리한다. 방어 심화용이며 보안 경계는 아니다. |
