...
| `guard_pim_board_command.py` | Claude Bash PreToolUse 가드. `env` option cluster·`builtin`·`exec`·`eval`·`source`·`nice`·`stdbuf`·`xargs`·`find`·`flock`·`setarch`·`start-stop-daemon`·`chroot`·`watch`·`taskset`·`chrt`·`ionice`·`script`·`prlimit`·`setsid`·`unshare`·`sudo` 등 launcher·shell `-c`·Bash ANSI-C quote·stdin shell fail-closed·명령 치환·그룹/조건 제어문 내부의 직접 plan/standalone runner 실행도 차단한다. `source`/`.`의 선택적 `--` 뒤 runner, `find`의 실행 action과 placeholder 가능성, `flock`의 직접 argv와 `-c` shell command, `setarch` 및 설치된 architecture 별칭, `start-stop-daemon --start`의 `--startas`/`--exec` 프로그램, exact `/` 대상 `chroot`의 command, `watch -x`의 exec 자식과 기본 shell command, `taskset`의 affinity, `chrt`의 priority, `ionice`의 class 옵션, `script -c`의 command 문자열, `prlimit`의 resource 옵션 및 `unshare`의 namespace 옵션 뒤 실행 자식을 재귀 검사하며 정확한 저장소 wrapper 경로만 면제한다. 실제 명령 앞의 shell redirection과 붙은/분리된 대상도 건너뛴 뒤 분류한다. 앞선 `cd`·`pushd`·`popd`, `start-stop-daemon`, 기본 `chroot` chdir 뒤에는 상대 wrapper 경로를 면제하지 않고 절대 정본 경로만 허용한다. 숫자 FD-only `flock`과 PID·PGID·UID 대상 모드는 실행 자식 없음으로 구분하고, 대화형 `script`, PID형 `prlimit`의 잔여 command, program 없는 `setarch`·`start-stop-daemon --start`·`chroot`·`unshare`, alternate-root/chroot 시작, 대상 없는 redirection·모호한 descriptor 복제형, ANSI-C escape·미종결 action·비정규 경로·축약 불가능한 복합 문법은 fail-closed 처리한다. 방어 심화용이며 보안 경계는 아니다. |
