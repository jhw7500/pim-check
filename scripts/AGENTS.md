...
| `guard_pim_board_command.py` | Claude Bash PreToolUse 가드. `env` option cluster·`exec`·`eval`·`source`·`nice` 등 launcher·shell `-c`·명령 치환·그룹/조건 제어문 내부의 직접 plan/standalone runner 실행도 차단하고, 축약 불가능한 복합 문법은 fail-closed 처리한다. 방어 심화용이며 보안 경계는 아니다. |
